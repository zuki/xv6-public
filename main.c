...
startothers(void)
{
  ...
  for(c = cpus; c < cpus+ncpu; c++){
    ...
    // entryother.Sに伝える。APプロセッサは低位アドレスで実効しているので
...
// entry.S と entryother.Sで使われるブートページテーブル.
// ページディレクトリ（とページエントリ）はページ境界から始めなければならない、
// そのため __aligned__ attribute を指定する。
// ページディレクトリエントリの PTE_PS はページサイズを4Mバイトにする。

__attribute__((__aligned__(PGSIZE)))
pde_t entrypgdir[NPDENTRIES] = {
  // 仮想アドレス [0, 4MB) を物理アドレス [0, 4MB) にマップ
  [0] = (0) | PTE_P | PTE_W | PTE_PS,
  // 仮想アドレス [KERNBASE, KERNBASE+4MB) を物理アドレス [0, 4MB)
