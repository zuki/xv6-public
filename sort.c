...
int cmp(char *, char *, FIELD *);
int compare(char *, char *);
void copy(char *, char *);
int digits(char *, char *, BOOL);
void error(BOOL, char *, char *);
void files_merge(int);
char *file_name(), *skip_fields();
void get_file(int, uint);
void incr(int, int);void
incr(register int si, register int);
int last_line(void);
int length(char *);
void merge(int, int);
void mread(int, char *, int);
void mwrite(int, char *, int);
void put_line(char *);
int read_line(MERGE *);
 ...
 */
void
get_opts(register char *ptr, register FIELD *field)
{
  switch (*ptr) {
...
atoi(register char *ptr)
{
  ...
  while (table[(int)*ptr] & DIGIT) num = num * 10 + *ptr++ - '0';
 ...
 *
 * @param[inout] Field to assign
 * @parma[inout] Offset in argv structure
 * @param[in] Assign beg or end of field
 */
void
new_field(register FIELD *field, int *offset, BOOL beg_fl)
{
  register char *ptr;
  ...
  *offset += 1;      /* Incr offset to next arg */
  ...
  if (beg_fl) {      /* Check for end pos */
    ptr = argptr[*offset];
    if (ptr && *ptr == '-' && table[(int)*(ptr + 1)] & DIGIT) {
  ...
  if (arg_count == argc) {  /* No args left. Use stdin */
    ...
    while (arg_count < argc) {  /* Sort or check args */
      if (strcmp(argv[arg_count], "-") == 0)
        fd = 0;
      else if (xv6_stat(argv[arg_count], &st) < 0) {
...
void
error(register BOOL quit, register char *message, register char *arg)
{
...
open_outfile(void)
{
  if (output_file == NIL_PTR)
    out_fd = STD_OUT;
  else if ((out_fd = open(output_file, O_WRONLY|O_CREATE)) < 0)
 ...
 *
 * @param[in] Fd of file to read
 * @param[in] Size of file
 */
void
get_file(int fd, register uint size)
{
  register int i;
  uint rest;         /* Rest in memory */
  ...
  if (fd == 0) {    /* We're reding stdin */
   ...
   /* Reading file. Check size */
    ...
    (void) lseek(fd, (int) (i - MEMORY_SIZE), SEEK_CUR);  /* Do this next time */
...
print_table(int fd)
{
  ...
  if (fd == ERROR) {
    if ((fd = open(file_name(nr_of_files), O_WRONLY|O_CREATE)) < 0)
...
void
mread(int fd, char *address, register int bytes)
{
...
void
mwrite(int fd, char *address, register int bytes)
{
...
void
incr(register int si, register int ei)
{
  char *tmp;
 ...
 */
int
cmp_fields(register char *el1, register char *el2)
{
  int i, ret = 0;
 ...
 *
 * @param[out] Holds result
 * @param[in] Field description
 * @param[in] Source line
 */
void
build_field(char *dest, register FIELD *field, register char *src)
{
...
char *
skip_fields(register char *str, int nf)
{
  while (nf-- > 0) {
    if (separator == '\0') {   /* Means ' ' or '\t' */
      while (table[(int)*str] & BLANK) str++;
      while (*str != ' ' && *str != '\t' && *str != '\n') str++;
    } else {
 ...
 */
int
compare(register char *el1, register char *el2)
{
  int ret;
 ...
 */
int
cmp(register char *el1, register char *el2, FIELD *field)
{
  int c1, c2;

  if (field->blanks) {    /* Skip leading blanks */
    while (table[(int)*el1] & BLANK) el1++;
    while (table[(int)*el2] & BLANK) el2++;
  }
  ...
  for (;;) {
    ...
    if (field->ascii) {  /* Skip chars outside 040 - 0177 */
      if ((table[(int)*el1] & ASCII) == 0) {
        do {
          el1++;
        } while ((table[(int)*el1] & ASCII) == 0);
        continue;
      }
      if ((table[(int)*el2] & ASCII) == 0) {
        do {
          el2++;
        } while ((table[(int)*el2] & ASCII) == 0);
        continue;
      }
    }
    if (field->dictionary) {/* Skip non-dict chars */
      if ((table[(int)*el1] & DICT) == 0) {
        do {
          el1++;
        } while ((table[(int)*el1] & DICT) == 0);
        continue;
      }
      if ((table[(int)*el2] & DICT) == 0) {
        do {
          el2++;
        } while ((table[(int)*el2] & DICT) == 0);
        continue;
      }
    }
 ...
 * @param[in] string1
 * @param[in] string2
 * @param[in] True if sign must be checked
 */
int
digits(register char *str1, register char *str2, BOOL check_sign)
{
...
/* Keep incrementing as long as digits are available and equal */
  while ((table[(int)*str1] & DIGIT) && table[(int)*str2] & DIGIT) {
    if (*str1 != *str2) break;
    str1++;
    str2++;
  }
  ...
  if (*str1 == '.' || *str2 == '.') {
    if (*str1 == '.') {
      if (*str2 == '.')  /* Both. Check decimal part */
        ...
        ret = (table[(int)*str2] & DIGIT) ? LOWER : HIGHER;
    } else
      ret = (table[(int)*str1] & DIGIT) ? HIGHER : LOWER;
  }

/* Now either two digits differ, or unknown char is seen (e.g. end of string) */
  else if ((table[(int)*str1] & DIGIT) && (table[(int)*str2] & DIGIT)) {
    diff = *str1 - *str2;  /* Basic difference */
    pow = 0;    /* Check power of numbers */
    while (table[(int)*str1++] & DIGIT) pow++;
    while (table[(int)*str2++] & DIGIT) pow--;
    ret = (pow == 0) ? diff : pow;
  }

/* Unknown char. Check on which string it occurred */
  else {
    if ((table[(int)*str1] & DIGIT) == 0)
      ret = (table[(int)*str2] & DIGIT) ? LOWER : SAME;
    else
      ret = HIGHER;
  }
 ...
 *
 * @param[in] Nr_of_files to merge
 */
void
files_merge(register int file_cnt)
{
  register int i;
  int limit;
  ...
  for (i = 0; i < file_cnt; i += OPEN_FILES) {
    ...
    if ((limit = i + OPEN_FILES) >= file_cnt) {
      ...
      if ((out_fd = open(temp_files, O_WRONLY|O_CREATE)) < 0)
...
{
  register MERGE *smallest = 0;  /* Keeps track of smallest line */
  register int i;
...
/* Calculate size in core available for file_cnt merge structs */
  ...
  free(mem_top);    /* First reset mem to lowest loc. */
 ...
 *
 * @param[in] merg structure to print
 * @param[in] Nr of files that are being merged
 */
MERGE *
print(register MERGE *merg, int file_cnt)
{
  register int i;
 ...
 */
MERGE *
skip_lines(register MERGE *smallest, int file_cnt)
{
  register int i;
  int ret;
...
void
copy(register char *dest, register char *src)
{
  while ((*dest++ = *src++) != '\n');
}
...
char *
msbrk(register int size)
{
  extern void *malloc(int);
  register char *address;

  if ((address = (char *)malloc(size)) == (char *) -1)
