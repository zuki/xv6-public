...
// ロックを長時間保持すると、他のCPUがロックの
// 獲得のためにスピンして時間を浪費する可能性がある。
void
...
// Pushcli/popcliは両者の数が一致することを除いて、cli/stiと同じである:
// 2回のpushcliを取り消すには2回のpopcliが必要である。また、割り込みが
// 無効であれば、pushcliとpopcliは割り込みを無効のままにする。
